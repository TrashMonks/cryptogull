...
def drawdialogueclassic(draw, imgdim, padding, charsize, title):
    #draw border
    draw.rectangle([(padding+1, padding+charsize[0]/2), (imgdim[0]-padding-2, imgdim[1]-padding-1)], outline='#b1c9c3', width=4)
    #draw person speaking. If title isn't specified, don't draw
    if title is not None and title != '':
        textdim = font.getsize(f'[ {title} ]')
        draw.rectangle([(charsize[0]*2+padding,0),(charsize[0]*2+padding+textdim[0],textdim[1]+padding)], fill='#0f3b3a')
        draw.text((charsize[0]*2+padding,0),'[', font=font, fill='#b1c9c3')
        draw.text((charsize[0]+padding+textdim[0],0),']', font=font, fill='#b1c9c3')
        draw.text((charsize[0]*2+padding+font.getsize('[ ')[0],0), title, font=font, fill='#cfc041')
    return draw

def test():
    return drawttf("There are hostiles nearby!")
    #return drawttf("""Qud is a strange and terrifying mesa to the northeast. Her tainted rivers breed life in {{G|all}} its motley forms; her poisoned jungles shelter priceless relics of a forgotten past.But that is just the half of it, for Qud's most precious treasures -- and her most hideous children -- lie within the innumerable chrome caverns beneath the scarlet loam.To ply those silver hollows -- a spry adventurer's dream! The years have wizened me beyond such foolish ambitions, but, you! Be not deterred so!""")
